...
export async function POST(_: Request, { params }: { params: Promise<{ provider: string }> }) {
  ...
  await admin.from("payment_connections").update({
    ...
    metadata: { ...(connection?.metadata || {}), cutflow_disabled: true },
    disconnected_at: new Date().toISOString(),
    last_error: provider === "stripe" ? "Stripe checkout is disabled in CutFlow. The barber's Stripe account remains open." : null,
  }).eq("barber_id", ctx.barber.id).eq("provider", provider);
