  ...
  return (
    <main className="storefront-site" style={{ "--barber-accent": barber.accent } as CSSProperties}>
      ...
      <section className={barber.coverImageUrl ? "storefront-hero has-cover" : "storefront-hero"} style={heroStyle}>
        <div className="storefront-hero-copy"><p className="eyebrow">PROFESSIONAL BARBERING · EASY ONLINE BOOKING</p><h1>{barber.headline}</h1><p>{barber.bio}</p><div className="storefront-actions"><Link className="button large" href={`/book/${slug}`}>Book an appointment <ArrowRight size={18}/></Link><a className="button large secondary" href="#services">View services</a></div><div className="storefront-proof"><span><ShieldCheck/>{money(barber.depositCents ?? 1000)} deposit credited to your total</span><span><CalendarCheck2/>Confirmed date and time</span><span><Sparkles/>Preferences saved for return visits</span></div></div>
      ...
      <section className="booking-embed-section"><header><p className="eyebrow">BOOK IN A FEW SIMPLE STEPS</p><h2>Choose your service, appointment time and preferences, then complete your deposit securely.</h2></header><BookingWizard embedded barber={barber} serviceItems={services} productItems={products}/></section>
