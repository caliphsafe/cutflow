  ...
  return (
    <main className="storefront-site" style={{ "--barber-accent": barber.accent } as CSSProperties}>
      <nav className="storefront-nav">
        <Link href={`/b/${slug}`} className="storefront-wordmark">{barber.logoImageUrl ? <img src={barber.logoImageUrl} alt={`${barber.shopName} logo`}/> : null}<span>{barber.shopName}</span></Link>
        <div><a href="#services">Services</a><a href="#products">Products</a><a href="#about">About</a><Link className="button small" href={`/book/${slug}`}>Book now</Link></div>
      </nav>

      <section className={barber.coverImageUrl ? "storefront-hero has-cover" : "storefront-hero"} style={heroStyle}>
        <div className="storefront-hero-copy"><p className="eyebrow">PRIVATE BARBERING · CLEAR BOOKING</p><h1>{barber.headline}</h1><p>{barber.bio}</p><div className="storefront-actions"><Link className="button large" href={`/book/${slug}`}>View availability <ArrowRight size={18}/></Link><a className="button large secondary" href="#services">View services</a></div><div className="storefront-proof"><span><ShieldCheck/>{money(barber.depositCents ?? 1000)} deposit applied to total</span><span><CalendarCheck2/>Exact appointment time</span><span><Sparkles/>Preferences remembered</span></div></div>
