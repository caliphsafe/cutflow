...
export default function SettingsPage() {
  ...
  return <div className="dashboard-page settings-page">
    ...
    <section className="settings-layout"><nav className="settings-nav dashboard-card"><a href="#profile" className="active"><UserRound/> Profile</a><a href="#payments"><CreditCard/> Payments</a><a href="#availability"><Clock3/> Availability</a><a href="#notifications"><Bell/> Notifications</a><a href="#security"><LockKeyhole/> Security</a></nav><div className="settings-sections">
      ...
      <article id="payments" className="dashboard-card settings-card"><header><span><WalletCards/></span><div><p className="eyebrow">PAYMENTS & PAYOUTS</p><h2>Stripe payment setup</h2></div></header><div className="connect-card"><span className="stripe-mark">+</span><div><b>Stripe customer payments</b><p>Complete Stripe’s secure onboarding, verify payout readiness and begin accepting customer deposits.</p></div><span className="connection-status pending"><Clock3/>Managed in Connections</span></div><div className="payment-settings-grid"><label><span>Booking deposit</span><div className="money-input"><span>$</span><input value={(profile.depositCents / 100).toFixed(2)} readOnly/></div><small>Credited toward the final appointment balance.</small></label><label><span>Customer payment destination</span><input value="Selected barber payment account" readOnly/><small>CutFlow does not add an appointment application percentage.</small></label></div><div className="fee-explainer"><ShieldCheck/><div><b>Barbers never enter API keys.</b><p>CutFlow stores platform credentials. The barber presses Connect, completes Stripe’s secure setup and returns to the dashboard.</p></div></div><footer><Link className="button" href="/dashboard/connections"><CreditCard/> Open payment connections</Link></footer></article>
